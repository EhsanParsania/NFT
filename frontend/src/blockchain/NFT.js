...
export const mintNFT = async (recipient, imageUrl) => {
    ...
    const tx = await NFT_CONTRACT.methods.mintNFT(recipient, imageUrl).send({ from: accounts[0] })
    return tx
}
...
export const lastNFTId = async () => {
    const lastId = await NFT_CONTRACT.methods._tokenIds().call()
    return lastId
}
